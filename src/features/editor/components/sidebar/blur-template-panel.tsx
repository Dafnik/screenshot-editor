...
export function BlurTemplatePanel() {
  ...
  const [isOpen, setIsOpen] = useState(() => blurTemplates.length > 0);
  ...
  return (
    <div className="border-border border-b-2">
      <button
        type="button"
        className="border-border flex w-full items-center justify-between border-b-2 p-4 text-left"
        onClick={() => setIsOpen((value) => !value)}
        aria-expanded={isOpen}
        aria-controls="blur-templates-content"
      >
                    ...
                    <Fragment key={template.id}>
                      ...
                      >
                        ...
                        <div className="text-muted-foreground mt-1 text-[10px]">
                          {template.strokes.length} stroke
                          {template.strokes.length === 1 ? '' : 's'}
                        </div>
